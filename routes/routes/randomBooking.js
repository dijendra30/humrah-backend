...
router.get('/chats/:chatId/messages', auth, async (req, res) => {
  try {
    ...
    // ✅ FIX: After .populate(), p.userId is a full User object, not a raw ObjectId.
    // Must use p.userId._id (if populated) or p.userId (if not populated).
    ...
    console.log('   chat participants:', chat.participants.map(p => p.userId?._id ? p.userId._id.toString() : p.userId.toString()));
