...
router.put('/me/mood', authenticate, async (req, res) => {
  try {
    ...
    // Upsert MatchingTodayMood — mood fields only, nearby untouched
    const dm = await MatchingTodayMood.findOneAndUpdate(
      { userId: req.userId },
      {
        $set: {
          mood,
...
router.get('/me/daily-mood', authenticate, async (req, res) => {
  try {
    const now = new Date();
    const dm  = await MatchingTodayMood.findOne({ userId: req.userId }).lean();
    const isActive = dm && dm.visible && dm.expiresAt && new Date(dm.expiresAt) > now;
